...
# Imports the required libDelphes, which is used to identify the objects in the ROOT file generated by delphes
ROOT.gSystem.Load("libDelphes.so")

class GenDataset():
    ''' 
    Gives methods to generate datasets from the root file made in delphes. 
    This module helps in taking the required information from the ROOT file and dump it into a HDF5 file
    ...
    createDataset()
        Generates the .h5 file from the datasetDict, reuires the createArray function to be run before
    ...
    def __init__(self,delphesRootFile : str,isRapid : bool = True):
        ...
        isRapid : bool
            True if the Delta R definition is to be take, False for absolute difference in azimuthal angle
    ...
    def createDataset(self,outputFile : str):
        '''
        Generate a HDF5 file from the datasetDict list. The first conversion is to split the dictionary 
        into 2 arrays, which are then converted to Awkward Arrays due to variable sizes of the particles 
        in an event. For the conversion from Awkward to HDF5, the default method provided by Awkward is used.
    ...
    def getPlots(self,arrayType : str,recordNo : int = 0):
        ...
        arrayType : str, required
            Specify which data is to be printed. "fourMomenta" for 4-momenta and "azimuthalAngle" for the azimuthal angle
